...
import { AMULETS, BELTS, BOOTS, BOWS, BRACERS, CHESTARMORS, CRUSADERSHIELDS, DAGGERS, FLAILS, GEMS, GLOVES, HELMS, PANTS, RELICS, RINGS, SHIELDS, SHOULDERS, SWORDS, TWOHANDEDFLAILS, TWOHANDEDMACES } from '../entities/crusader'

@Component({
  selector: 'app-crusader',
  templateUrl: './crusader.component.html',
  styleUrls: ['./crusader.component.css']
})
export class CrusaderComponent {
  ...
  @ViewChild('flailsAccordion') flailsAccordion!: MatAccordion;
  @ViewChild('swordsAccordion') swordsAccordion!: MatAccordion;
  @ViewChild('twohandedflailsAccordion') twohandedflailsAccordion!: MatAccordion;
  @ViewChild('twohandedmacesAccordion') twohandedmacesAccordion!: MatAccordion;
  @ViewChild('crusadershieldsAccordion') crusadershieldsAccordion!: MatAccordion;
  ...
  countMissing(count: number, build: number): number {
    ...
    count += countMissingBuild(FLAILS, build);
    count += countMissingBuild(SWORDS, build);
    count += countMissingBuild(TWOHANDEDFLAILS, build);
    count += countMissingBuild(TWOHANDEDMACES, build);
    count += countMissingBuild(CRUSADERSHIELDS, build);
  ...
  itemsMissing(threshold: number): number {
    let items = HELMS.concat(SHOULDERS).concat(CHESTARMORS).concat(GLOVES).concat(PANTS).concat(BOOTS)
      ...
      .concat(BOWS).concat(DAGGERS).concat(FLAILS).concat(SWORDS).concat(TWOHANDEDFLAILS).concat(TWOHANDEDMACES).concat(CRUSADERSHIELDS).concat(SHIELDS).concat(RELICS);
...
`;
    ...
    // Flails
    data += `export const FLAILS : Item[] = `;
    data += JSON.stringify(this.flails, null, 2);
    data += `
...
`;
    ...
    // Two-Handed Flails
    data += `export const TWOHANDEDFLAILS : Item[] = `;
    data += JSON.stringify(this.twohandedflails, null, 2);
    data += `

`;

    // Two-Handed Maces
    data += `export const TWOHANDEDMACES : Item[] = `;
    data += JSON.stringify(this.twohandedmaces, null, 2);
    data += `

`;

    // Crusader Shields
    data += `export const CRUSADERSHIELDS : Item[] = `;
    data += JSON.stringify(this.crusadershields, null, 2);
    data += `
...
`;
  ...
  openAll() {
    ...
    this.flailsAccordion.openAll();
    this.swordsAccordion.openAll();
    this.twohandedflailsAccordion.openAll();
    this.twohandedmacesAccordion.openAll();
    this.crusadershieldsAccordion.openAll();
  ...
  closeAll() {
    ...
    this.flailsAccordion.closeAll();
    this.swordsAccordion.closeAll();
    this.twohandedflailsAccordion.closeAll();
    this.twohandedmacesAccordion.closeAll();
    this.crusadershieldsAccordion.closeAll();
  ...
  flails = FLAILS;
  swords = SWORDS;
  twohandedflails = TWOHANDEDFLAILS;
  twohandedmaces = TWOHANDEDMACES;
  crusadershields = CRUSADERSHIELDS;
  shields = SHIELDS;
  relics = RELICS;
  gems = GEMS;
